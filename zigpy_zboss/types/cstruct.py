...
class CStructField:
    """Class for Cstruct field."""

    name: str
    type: type

    def __post_init__(self) -> None:
        """Set parameters about the cstruct."""
        # Throw an error early
        self.get_size_and_alignment()

    def get_size_and_alignment(self, align=False) -> tuple[int, int]:
        """Return size and alignment of a cstruct."""
        if issubclass(self.type, zigpy_t.FixedIntType):
            assert self.type._size is not None
            ...
            return self.type.get_size(align=align), \
                self.type.get_alignment(align=align)
...
class CStruct:
    """Class represeting cstruct."""

    _padding_byte = b"\xFF"

    def __init_subclass__(cls):
        """Set cstruct fields when used as parent class."""
    ...
    def as_dict(self) -> dict[str, typing.Any]:
        """Return a dict representation of a Cstruct."""
    ...
    def deserialize(cls, data: bytes, *, align=False) -> tuple[CStruct, bytes]:
        ...
        if orig_length < expected_size:
            raise ValueError(
                f"Data is too short, must be at least {expected_size} "
                f"bytes: {data!r}"
            )
    ...
    def __eq__(self, other: CStruct) -> bool:
        """Return True if the dict representation of two Cstruct are equal."""
    ...
    def __repr__(self) -> str:
        """Return a Cstruct representation."""
